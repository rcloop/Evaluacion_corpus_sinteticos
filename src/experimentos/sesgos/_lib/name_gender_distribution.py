...
# Solo etiquetas en castellano (MEDDOCAN). Para corpus con etiquetas en inglés, pasar target_labels explícitamente.
...
# Lexicón canónico: si existe, todos los experimentos lo usan por defecto (sin --lexicon_path).
_REPO_ROOT = Path(__file__).resolve().parents[4]
DEFAULT_LEXICON_PATH = _REPO_ROOT / "data" / "sesgos" / "name_gender_lexicon.json"
...
def load_lexicon(path: Optional[str]) -> Dict[str, str]:
    ...
    When path is None, uses DEFAULT_LEXICON_PATH if it exists (data/sesgos/name_gender_lexicon.json),
    so the reviewed classification is used by all scripts. Otherwise falls back to built-in lexicon.

    Accepted formats:
    - JSON grouped: { "fem": [...], "masc": [...], "other": [...] } (replaces default).
      "other" = nombres que quieres dejar explícitamente como no clasificados.
    - JSON flat: { "MARIA": "fem", "JOSE": "masc", ... } (overlays on default)
    - CSV with columns: name/nombre, gender/genero (fem/masc; overlays on default)
    """
    effective_path = path
    if effective_path is None and DEFAULT_LEXICON_PATH.exists():
        effective_path = str(DEFAULT_LEXICON_PATH)
    ...
    if p.suffix.lower() == ".json":
        ...
        if not isinstance(data, dict):
            raise ValueError("Lexicon JSON must be a dict.")
        # Grouped format (review file): full replace
        if "fem" in data and "masc" in data and isinstance(data["fem"], list) and isinstance(data["masc"], list):
            base = {}
            for n in data["fem"]:
                key = normalize_name(str(n))
                if key:
                    base[key] = "fem"
            for n in data["masc"]:
                key = normalize_name(str(n))
                if key:
                    base[key] = "masc"
            for n in data.get("other", []):
                key = normalize_name(str(n))
                if key:
                    base[key] = "other"
            return base
        # Flat format: overlay on default
        base = _default_lexicon().copy()
        for k, v in data.items():
            if k in ("fem", "masc", "description", "count_fem", "count_masc") or (k == "other" and isinstance(v, list)):
                continue
            nk = normalize_name(k)
    ...
    if p.suffix.lower() in {".csv", ".tsv"}:
        ...
        with p.open("r", encoding="utf-8-sig", newline="") as f:
            ...
            name_col = "name" if "name" in fields else "nombre" if "nombre" in fields else None
            gender_col = "gender" if "gender" in fields else "genero" if "genero" in fields else None
            if not name_col or not gender_col:
                raise ValueError("Lexicon CSV must have columns: name, gender (or nombre, genero)")
            for row in reader:
...
def infer_gender(
    ...
    if key == "PACIENTE":
        ...
        return "other"
    if key == "LA":
        # "La paciente" / "La paciente ..." → fem (artículo femenino)
        if full_name:
            n_full = normalize_name(full_name)
            if "PACIENTE" in n_full or _full_text_suggests_female(full_name):
                return "fem"
        return "other"
...
def iter_documents(
    ...
    if p.is_dir():
        ...
        if max_files is not None:
            files = files[: max_files]
        for fp in tqdm(files, desc="Documentos (name_gender)", unit="doc"):
            try:
                obj = json.loads(fp.read_text(encoding="utf-8"))
            except Exception:
                continue
...
def wilson_score_ci(successes: int, n: int, z: float = 1.96) -> Tuple[float, float]:
    """Wilson score interval for a proportion (95% CI by default). Returns (lower, upper)."""
    if n == 0:
        return (0.0, 0.0)
    p = successes / n
    denom = 1 + z * z / n
...
def chi_square_uniformity(counts: Dict[str, int]) -> Dict[str, Optional[float]]:
    ...
    try:
        ...
        p_value = None
    # Cohen's w for goodness of fit: effect size (0 = fit, larger = more deviation)
    w = (chi2 / total) ** 0.5 if total else None

    return {"chi2": float(chi2), "p_value": p_value, "df": float(k - 1), "cohens_w": w}
...
def collect_other_names(
    ...
    """
    Recorre el corpus con la misma lógica que evaluate_name_gender_distribution y devuelve
    la lista de nombres que quedaron clasificados como "other", para revisión.
    Cada elemento: {"first_name": str, "full_text": str, "entity_label": str}.
    """
        ...
        if subject_names:
            for i, name in enumerate(subject_names):
                ...
                if g == "other":
                    first = extract_first_name(name)
                    out.append({
                        "first_name": normalize_name(first) if first else "",
                        "full_text": name,
            ...
            for name in personnel_names:
                ...
                if g == "other":
                    ...
                    out.append({
                        "first_name": normalize_name(first) if first else "",
                        "full_text": name,
                        "entity_label": "NOMBRE_PERSONAL_SANITARIO",
                    })
...
def evaluate_name_gender_distribution(
    ...
    def proportions_ci(c: Counter) -> Dict[str, List[float]]:
        """95% Wilson score intervals for each proportion."""
        n = sum(c.values())
        if n == 0:
            return {"p_fem_ci": [0.0, 0.0], "p_masc_ci": [0.0, 0.0], "p_other_ci": [0.0, 0.0]}
        return {
            "p_fem_ci": list(wilson_score_ci(c.get("fem", 0), n)),
            "p_masc_ci": list(wilson_score_ci(c.get("masc", 0), n)),
            "p_other_ci": list(wilson_score_ci(c.get("other", 0), n)),
        }
    ...
    for lu in sorted(target_set):
        ...
        per_label[lu] = {
            ...
            "proportions": p,
            "proportions_ci_95": proportions_ci(c),
    ...
    overall = {
        ...
        "proportions_ci_95": proportions_ci(overall_counter),
    ...
    effective_lexicon_path = lexicon_path if lexicon_path else (str(DEFAULT_LEXICON_PATH) if DEFAULT_LEXICON_PATH.exists() else None)
    return {
        ...
        "input_summary": {
            ...
            "lexicon_path_effective": effective_lexicon_path,
