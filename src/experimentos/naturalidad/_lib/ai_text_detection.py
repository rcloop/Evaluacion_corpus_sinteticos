...
def load_corpus(corpus_path: str) -> List[str]:
    ...
    if os.path.isdir(corpus_path):
        ...
        for file_path in tqdm(files, desc="AI detection (documents)", unit="doc"):
...
def prepare_classification_data(
    ...
    # Vectorize (min_df/max_df adaptados a corpus pequeños para tests)
...
def evaluate_ai_detection(
    ...
    if min_size < min_per_class_for_classifier:
        ...
        print(f"\n[OK] Insufficient data (min {min_per_class_for_classifier} texts per class). Results saved to: {output_path}")
        return results
