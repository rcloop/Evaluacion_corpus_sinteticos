...
def load_corpus(corpus_path: str) -> List[str]:
    ...
    if os.path.isdir(corpus_path):
        ...
        for file_path in tqdm(files, desc="Coherence (documents)", unit="doc"):
...
def evaluate_coherence(
    ...
    try:
        ...
        _device = "cpu"
    print(f"\n2. Loading sentence transformer model: {model_name} (device: {_device})")
    try:
    ...
    # Statistics
    n_coh = len(coherence_scores)
    mean_coh = float(np.mean(coherence_scores))
    std_coh = float(np.std(coherence_scores))
    margin_coh = 1.96 * (std_coh / (n_coh ** 0.5)) if n_coh > 0 else 0.0
    mean_ci_95 = [mean_coh - margin_coh, mean_coh + margin_coh]
