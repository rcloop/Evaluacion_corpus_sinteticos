...
def load_corpus(corpus_path: str) -> List[str]:
    ...
    if os.path.isdir(corpus_path):
        ...
        for file_path in tqdm(files, desc="Perplexity (documents)", unit="doc"):
...
def calculate_perplexity_mlm(text: str, model, tokenizer, max_length: int = 512, device=None) -> float:
    """
    Calculate perplexity using masked language model.
    
    Args:
        text: Input text
        model: Pre-trained MLM model (already on device)
        tokenizer: Tokenizer
        max_length: Maximum sequence length
        device: torch device for inputs (if None, uses model device)
...
def calculate_perplexity_causal(text: str, model, tokenizer, max_length: int = 512, device=None) -> float:
    """
    Calculate perplexity using causal language model.
    
    Args:
        text: Input text
        model: Pre-trained causal LM model (already on device)
        tokenizer: Tokenizer
        max_length: Maximum sequence length
        device: torch device for inputs (if None, uses model device)
...
def evaluate_perplexity(
    ...
    for i, text in enumerate(texts):
        ...
        try:
            ...
            perplexity = calculate_perplexity(text, model, tokenizer, max_length, device)
    ...
    n_pp = len(perplexities)
    mean_pp = float(np.mean(perplexities))
    std_pp = float(np.std(perplexities))
    # 95% CI for the mean (normal approximation)
    margin_pp = 1.96 * (std_pp / (n_pp ** 0.5)) if n_pp > 0 else 0.0
    mean_ci_95 = [mean_pp - margin_pp, mean_pp + margin_pp]
