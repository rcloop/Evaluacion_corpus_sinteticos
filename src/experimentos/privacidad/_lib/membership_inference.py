...
def load_corpus(corpus_path: str) -> List[str]:
    ...
    if os.path.isdir(corpus_path):
        ...
        for file_path in tqdm(files, desc="Membership inference (documents)", unit="doc"):
...
def train_membership_inference_attack(
    ...
    """
    # Split data (stratify solo si hay ≥2 clases y cada una con ≥2 muestras)
    ...
    auc_ci = _bootstrap_auc_ci(y_test, y_pred_proba)
