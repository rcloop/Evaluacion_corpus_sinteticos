...
def load_corpus_with_annotations(corpus_path: str, annotations_path: str = None) -> Tuple[List[str], List[Dict]]:
    ...
    if os.path.isdir(corpus_path):
        ...
        for txt_file in tqdm(txt_files, desc="Attribute inference (documents)", unit="doc"):
...
def _bootstrap_auc_ci(y_true: np.ndarray, y_proba: np.ndarray, n_bootstrap: int = 500, seed: int = 42) -> List[float]:
    """95% CI for AUC-ROC via bootstrap. Returns [lower, upper] or [None, None] if not computable."""
...
def train_attribute_inference_attack(
    ...
    # After split, need at least 2 train samples and both classes for LogisticRegression
    if X_train.shape[0] < 2 or len(np.unique(y_train)) < 2:
        return None, {
            'attribute': attribute_name,
            'accuracy': 0.0,
            'auc_roc': 0.0,
            'auc_roc_ci_95': [None, None],
            'positive_rate': np.mean(y),
            'baseline_accuracy': max(np.mean(y), 1 - np.mean(y)),
            'skipped': True,
            'reason': 'Insufficient samples after train/test split for corpus mini'
        }
    ...
    # Calculate metrics (AUC requires both classes in y_test)
    ...
    # 95% CI for AUC-ROC (bootstrap)
    auc_ci = _bootstrap_auc_ci(y_test, y_pred_proba) if len(np.unique(y_test)) > 1 else [None, None]
    metrics = {
        'attribute': attribute_name,
        'accuracy': accuracy,
        'auc_roc': auc_score,
...
def evaluate_attribute_inference(
    ...
    # Get all attribute names (from first document; all have same keys)
    if not all_attributes:
        results = {
            'corpus_size': 0,
            'attributes_evaluated': [],
            'attribute_results': {},
            'overall_risk': {'max_auc_roc': 0.0, 'mean_auc_roc': 0.0, 'high_risk_attributes': []}
        }
        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump(results, f, indent=2, ensure_ascii=False)
        print("No texts loaded. Results saved to", output_path)
        return results
    attribute_names = list(all_attributes[0].keys())
    
    # Vectorize texts (min_df/max_df adaptados a corpus pequeños para tests)
    n_docs = len(texts)
    min_df = 1 if n_docs < 5 else 2
    max_df_val = 1.0 if n_docs < 5 else 0.95
    ...
    for attr_name in attribute_names:
        ...
        # Risk assessment (skip risk_level when attack was skipped for small corpus)
    ...
    # Overall risk assessment (handle all attributes skipped -> risk_scores empty)
    risk_scores = [
        r.get('auc_roc', 0) for r in results['attribute_results'].values()
        if isinstance(r, dict) and 'auc_roc' in r and not r.get('skipped')
    ]
    results['overall_risk'] = {
        'max_auc_roc': float(max(risk_scores)) if risk_scores else 0.0,
        'mean_auc_roc': float(np.mean(risk_scores)) if risk_scores else 0.0,
